...
os.system("sort -k 4,4 -s  /data/nooroka/grant/punkt3/stage2/gccoords/def/gccoords_{}2defnongenes_control_less_50_40.bed > /data/nooroka/grant/punkt3/stage2/gccoords/def/gccoords_{}_undefnongenes_control_less_50_40.bed".format(sys.argv[2],sys.argv[2]))
cmd = """bedtools intersect -a <(awk 'BEGIN{{OFS="\\t"}} {{ if ($3 <= $2) $3=$2+1; print }}' /data/nooroka/grant/punkt3/bed-37/bed_chr_{}_sorted.bed) -b /data/nooroka/grant/punkt3/stage2/gccoords/def/gccoords_{}_undefnongenes_control_less_50_40.bed -wa -wb | awk '!seen[$4]++' > /data/nooroka/grant/punkt3/stage2/intmut/intmuthg19{}_genome_control.bed""".format(
