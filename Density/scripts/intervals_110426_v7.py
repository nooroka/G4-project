...
out_file = "results_density_{}_{}.tsv".format(sys.argv[1], sys.argv[2])
...
with open(out_file, "w") as w:
    ...
    for i in range(1, 25):
        ...
        # дедупликация мутаций один раз на входе
        a_cmd = "<(zcat {mut} | awk -v OFS='\\t' '{{if ($3<=$2) $3=$2+1; print}}' | sort -u)".format(
            mut=mut_file)

        mut_gc = count_lines("bedtools intersect -a {a} -b <(zcat {gc}) -u".format(
            a=a_cmd, gc=gc_file))
        mut_b  = count_lines("bedtools intersect -a {a} -b {b} -u".format(
            a=a_cmd, b=b_file))
