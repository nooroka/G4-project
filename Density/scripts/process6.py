...
def remove_cg_dinucleotides_in_fasta(input_file, output_file, bed_file, chunk_size=1000000):
    ...
    Сохраняет координаты удалённых CG в BED-файл.
...
def sort_bed_file(input_bed, output_bed):
    """
    Сортирует BED-файл с помощью bedtools sort.

    :param input_bed: Путь к входному BED-файлу.
    :param output_bed: Путь к выходному отсортированному BED-файлу.
    """
    try:
        command = ["bedtools", "sort", "-i", input_bed]
        with open(output_bed, 'w') as outfile:
            subprocess.run(command, stdout=outfile, check=True)
        print(f"Отсортированный BED-файл сохранён в: {output_bed}")
    except subprocess.CalledProcessError as e:
        print(f"Ошибка при сортировке BED-файла: {e}")
...
if __name__ == "__main__":
    ...
    output_file = "../../cleaned_genome6.fasta"  # Файл для очищенной последовательности
    bed_file = "../../removed_cg_coordinates6.bed"  # Файл для координат удалённых CG
    sorted_bed_file = "../../removed_cg_coordinates6_sorted.bed"  # Отсортированный BED-файл
    chrom_sizes_file = "../../hg19.chrom6.sizes"  # Файл с размерами хромосом
    genome_bed_file = "../../genome_regions6.bed"  # BED-файл для всего генома
    subtracted_bed_file = "../../subtracted_regions6.bed"  # Файл для результата вычитания
    ...
    # Шаг 3: Создание BED-файла для всего генома
    create_genome_bed_file(chrom_sizes_file, genome_bed_file)

    # Шаг 4: Сортировка BED-файла с удалёнными CG
    sort_bed_file(bed_file, sorted_bed_file)

    # Шаг 5: Использование bedtools subtract
    run_bedtools_subtract(genome_bed_file, sorted_bed_file, subtracted_bed_file)
