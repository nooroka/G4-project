...
cmd1 = (
"""bedtools intersect """
"""-a <(zcat /data/nooroka/grant/punkt3/bed-37/bed_chr_{c}_sorted.bed.gz | """
"""awk 'BEGIN{{OFS="\\t"}} {{ if ($3 <= $2) $3=$2+1; print }}') """
"""-b <(sort -k1,1 -k2,2n {b}) """
"""-sorted -wa -wb """
"""> /data/nooroka/grant/punkt3/stage2/resultgene/"""
"""resultgenehg19{c}_39_all_loop7_control2_no_gc_corrected_plus.bed"""
...
w3 = open(
    ...
    "gccoords_{}2defhg19_39_all_loop7_control2_no_gc_corrected2_plus.bed".format(sys.argv[3]),
    "w"
)
a = ""
sum_control1 =0
...
#os.system(
 #   "sort -k1,1 -k2,2n /data/nooroka/grant/punkt3/stage2/gccoords/def/"
  #  "gccoords_{c}2defhg19_39_all_loop7_control2_no_gc_corrected.bed "
  #  "| awk '!seen[$1,$2,$3]++' "
  #  "> /data/nooroka/grant/punkt3/stage2/gccoords/def/"
  #  "gccoords_{c}_undefhg19_39_all_loop7_control2_no_gc_corrected.bed".format(c=sys.argv[3])
#)
...
os.system(
    ...
    "| awk '!seen[$4]++' "
    "> /data/nooroka/grant/punkt3/stage2/resultgene/"
    "resultgenehg19{c}_un_39_all_loop7_control2_no_gc_corrected_plus.bed".format(c=sys.argv[3])
)
...
gccoords_2def = (
    "/data/nooroka/grant/punkt3/stage2/gccoords/def/"
    "gccoords_{c}2defhg19_39_all_loop7_control2_no_gc_corrected2_plus.bed".format(c=sys.argv[3])
)
intmut_out = (
    "/data/nooroka/grant/punkt3/stage2/intmut/"
    "intmuthg19{c}_39_all_loop7_control2_no_gc_corrected_plus.bed".format(c=sys.argv[3])
)
...
bed_a_inner = (
    "awk 'BEGIN{{OFS=\"\\t\"}} {{ if ($3 <= $2) $3=$2+1; print }}' "
    "/data/nooroka/grant/punkt3/bed-37/bed_chr_{c}_sorted.bed.gz"
).format(c=sys.argv[3])

run_bedtools_chunked(bed_a_inner, gccoords_2def, intmut_out + ".raw", CHUNK_LINES)
...
d2 = count_lines_fast(
    "/data/nooroka/grant/punkt3/stage2/resultgene/"
    "resultgenehg19{}_un_39_all_loop7_control2_no_gc_corrected_plus.bed".format(sys.argv[3])
)
...
d1  = count_lines_fast(intmut_out)   # = d11 в оригинале
...
if sum_control1 == 0:
    ...
    w.write(
        ...
        "\taverage G4 motif/interval length\t{}\n".format(
            sys.argv[3], float(int(d11) / int(sum_control1)), a
        )
    )
